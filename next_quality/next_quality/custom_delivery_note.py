...
def get_list(self,method):
    for i in self.items:
        ...
        for j in count:
            if j.get('count')>0:
                abc=frappe.db.get_all("Customer Quality Inspection",{"sales_order":i.against_sales_order,"docstatus":1},["name"])
                for kj in abc:
                    doc=frappe.get_doc("Customer Quality Inspection",kj.name)
                    if doc.item==i.item_code:
                        bac=frappe.db.get_value("Batch",{"name":b},["name"])
                        if bac:
                            lst=frappe.get_doc("Batch",i.batch_no)
